 ...
 * Written by Jonathan Devor (CfA), 11 May 2005
 *
 ...
 *
 * 1. The binary orbits are CIRCULAR (i.e has no eccentricity ; e=0)
 *      - The angle of perihelion (ill-defined) is set to 0
 *
 * 2. Both stars are spherical:
 *      - Detached binary system --> negligible tidal bulge (in Roche lobe)
 *      - Slow rotation --> negligible centripetal deformation
 *
 * 3. Both stars have a Sun-like photosphere:
 ...
 *
 * Compile:  gcc debil_noEcc_noDoublePeriod.c -o debil_noEcc_noDoublePeriod -Wall -lm -O4
 * Run: debil_noEcc_noDoublePeriod p.txt p.out p.err
 *
 *******************************************************************************************/
...
#define DEFAULT_NUM_ITERATIONS 4000
...
#define ECCENTRICITY_WARINING_THESHOLD 0.05
...
#define DIM 6  // The number of dimensions (don't change!)

// list of dimensions:
// physical-
#define D_R1    0  // Star's radius (in units of the distance between them)
#define D_R2    1
#define D_B1    2  // Central surface brightness (flux per unit disk area)
#define D_B2    3
// orientation-
#define D_SIN_I 4  // sin(inclination)
#define D_TMO   5  // time - (omega / 2pi) = time  (since we set omega=0)
...
double scoreFit (double p[DIM], float *time, float *amp, float *err, int size, double dr)
{
  ...
  if ((p[D_R1] <= 0.0) || (p[D_R1] >= 1.0)) return (MAXFLOAT) ;               // r1
  if ((p[D_R2] <= 0.0) || (p[D_R2] >= (1.0 - p[D_R1]))) return (MAXFLOAT) ;   // r2
  ...
  if (p)
    {
      ...
      if ((p[D_R1] <= 0.0) || (p[D_R1] >= 1.0)) return(-3) ;               // r1
      if ((p[D_R2] <= 0.0) || (p[D_R2] >= (1.0 - p[D_R1]))) return(-4) ;   // r2
...
{
  int i ;
  double errR1, errR2, errTMO, errB1, errB2 ;
  ...
  if (i == DIM)
    {
      ...
      errTMO = estimateError(D_TMO, p, y, 1, time, amp, err, size, numIterations) ;
      ...
      fprintf (fout, "0.0 -1.0 ") ; // p[D_ECC], estimateError(D_ECC, p, y, 0, time, amp, err, size, numIterations)
      ...
      fprintf (fout, "%f %f ", mod1(p[D_TMO]), errTMO) ;
      fprintf (fout, "0.0 -1.0 ") ; // 360.0 * mod1(p[D_TPO] - p[D_TMO]), 360.0 * errTMPO
...
int main (int argc, char **argv)
{
  ...
  double sin_i, r1, r2, B1, B2, e, period, time0 ;
  double medianAmp, min1HalfWidth, min2HalfWidth, Y, varySin_i ;
  ...
  while (2 == fscanf(finList, "%s %lf\n", filename, &period))
      ...
      if ((e < 0.0) || (e >= 1.0))
	{
      ...
      if (e >= ECCENTRICITY_WARINING_THESHOLD)
	{
	  sprintf (str, "The light curve seems to show a large eccentricity (e=%f, tDiff=%f)", e, tDiff) ;
	  printError (argv[3], 0, filename, "main()", str) ;
	}
	...
	sin_i = getSin_i (time, amp, size, e, Y, r1, r2, &varySin_i,
      ...
      for (i = 0 ; i < DIM+1 ; i++)
	{
	  p[i][D_R1] = r1 - ((i == 1) * 0.1 * r1) ;
	  p[i][D_R2] = r2 - ((i == 2) * 0.1 * r2) ;
	  p[i][D_B1] = B1 + ((i == 3) * 0.1 * B1) ;
	  p[i][D_B2] = B2 + ((i == 4) * 0.1 * B2) ;
	  p[i][D_SIN_I] = sin_i - ((i == 5) * varySin_i) ;
	  p[i][D_TMO] = time0 + ((i == 6) * 0.01) ;
