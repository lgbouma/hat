/**************************************************************************
 * this program finds the period of a given OGLE-type light curve table
...
{
  int i, index ;
  double sum = 0.0, X, Y, s1 = 0.0, L2 = 0.0 ;
  unsigned long N ;
  ...
  for (i = 0 ; i < HIST_SIZE ; i++)
    {
      N = histN[i] ;
      if (N <= 1) return (ERROR_SCORE) ;
      Y = histD[i] ;
      X = Y * Y / N ;

      sum += Y ;
      s1 += X ;
      L2 += (sqr((histC[i] * N) - (Y * histA[i])) / (N * (sqr(histA[i]) - (histB[i] * N)))) + histE[i] - X ;
    }
...
int main(int argc, char **argv)
{
  ...
  double *time, *mag ;
  double sum = 0.0, sumSqr = 0.0, stddiv, tmpTime, tmpMag, outAvr, outStdDiv ;
  char str[32] ;
  FILE *fp, *fout ;
  ...
  size = 0 ;
  while (2 == fscanf(fp, "%lf %*f %*f %lf %*f %*d\n", &tmpTime, &tmpMag))
