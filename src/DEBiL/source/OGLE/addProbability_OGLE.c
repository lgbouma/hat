 ...
 *
 *
 * compile:  gcc addProbability_OGLE.c -o addProbability_OGLE -lm -Wall -O3
 *
 * run: 
 *       makeEffTable 100 100 10000 makeEffTable.res
 *      
...
int main(int argc, char **argv)
{
  ...
  double x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41, x42 ;
  int x1, x2, x20, x21, val ;
  ...
  if ((argc != 4) && (argc != 5))
    {
      printf ("Usage: %s <Eff table> <input DEBiL /w color> <output DEBiL /w color /w weight> [period histogram]\n", argv[0]) ;
      return (1) ;
    }
  ...
  */
  ...
  while (44 == fscanf (fin, "%d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %s %s\n",
		       &x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9, &x10, &x11, &x12, &x13, &x14, &x15, &x16, 
		       &x17, &x18, &x19, &x20, &x21, &x22, &x23, &x24, &x25, &x26, &x27, &x28, &x29, &x30, 
		       &x31, &x32, &x33, &x34, &x35, &x36, &x37, &x38, &x39, &x40, &x41, &x42, x43, x44))
    {
      p = avr2d(x6+x8 , x4, arr, numRa, numEcc) ; // (r1+r2, e, arr, numRa, numEcc)

      fprintf (fout, "%d %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %s %s %f\n",
	       x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, 
	       x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27, x28, x29, x30,
	       x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41, x42, x43, x44, 1.0 / p) ;
      
      if (histPeriod && fhist)
	{
	  val = (int)(log10(x3 * 10.0) * NUM_HIST / 4.0) ;      // transform [0.1, 1000] --> [0 , numHist]
