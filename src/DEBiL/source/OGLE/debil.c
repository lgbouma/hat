...
// If the period needs to be doubled, assume that the dips are very similar and only fit r1<r2 (not r1>r2)
...
#define VARIANCE_LIMIT 10.0    // Threshold variance (sigma squared)
...
// Returns the modulo 1 value (non-negative) of x
...
double ridOutliers (float *time, float *amp, float *err, int *psize)
{
  ...
  do
      ...
      while (index < (*psize))
	{
	  ...
	  if (variance > (varianceFit * VARIANCE_LIMIT))
	    {
	      doRid = 1 ;
	      (*psize)-- ;
  ...
  if (i == DIM)
    {
      ...
      fprintf (fout, "%s %f ", filename, period) ;
...
#ifdef WRITE_CURVE_FIT
  if ((y <= WRITE_CURVE_CHI2) && (sigmaMin2Amp >= WRITE_CURVE_STDDIV_BELOW_MEDIAN))
    {
...
int main (int argc, char **argv)
{
  ...
  while (2 == fscanf(finList, "%s %lf\n", filename, &period))
	...
	if ((tmpTime >= 0.0) && (tmpMag > 0.0) && (tmpErr > 0.0))  // Sign of an invalid magnitude
	  {
	    time[i] = (float)(fmod(tmpTime, period) / period) ;
	...
	{
	  printError (argv[3], 1, filename, "main()", "Hump is too large") ;
	  goto ABORT_LIGHT_CURVE_FITTING ;
	}
