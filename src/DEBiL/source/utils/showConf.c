/*********************************************************************
 *
 * This utility displays a list of light curves (3-column LCs) in ASCII form, 
 * wrapped around (modulo) a given period
 *
 * compile:  gcc showConf.c -o showConf -Wall -lm 
 * run: showConf ../periodFinder/scripts/big.txt 0 big.comment.txt
 *
 **********************************************************************/
...
int main(const int argc, const char **argv)
{
  ...
  int rid ;
  double period, conf, avr, stddiv ;
  char strUser[512], strFilename[512] ;
  double *time, *mag ;
  double tmpTime, tmpMag, tmpErr ;
  unsigned long numRid, size, prevSize, counter = 0, offset = 0 ;
  ...
  while (fscanf (finList, "%s %lf %lf %*f %*f %u %lf %lf\n", strFilename, &period, &conf, &rid, &avr, &stddiv) == 6)
    {
      counter++ ;
      if (counter < offset) continue ;

#ifdef MIN_CONFIDENCE
      ...
      if (!finLC)
	{
      ...
      printf ("\n\n #%lu %s  period= %f  (%f)\n\n", counter, strFilename, period, conf) ;
      ...
      if (prevSize != size)
	{
      ...
      if (fout)
	{
	  fprintf (fout, "%s %f %f %u %f %f", strFilename, period, conf, rid, avr, stddiv) ;
