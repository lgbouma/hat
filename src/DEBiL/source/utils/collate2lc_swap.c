/************************************************************
 *
 * This utility takes two DEBiL (31-param) databases. 
 * It scans through the first DB. If the given LC (by name + aprix. period) exists in the second LC, it uses the latter vesion.
 * If it doesn't exist, it skips over that LC.
 * This is used to swap single eclipse LCs (with better chi2), with ones of e=0 
 *
 * Compile:  gcc collate2lc_swap.c -o collate2lc_swap -Wall -lm
 *
 * Run:  collate2lc_swap collate.sort.select.eye.1dip.dbl collate_noEcc.sort.dbl > collate.sort.select.eye.1dip.noEcc.dbl
 *
 ************************************************************/
...
int main(int argc, char **argv)
{
  ...
  if (argc != 3)
    {
      printf (" Usage:  %s  <DEBiL database #1 (31-param) - scan>  <DEBiL database #2 (31-param) - use>\n", argv[0]) ;
      return (1) ;
    }
