 ...
 * wrapped around (modulo) a given period. Assumes a 2-columns format: [time ; mag]
 *
 * compile:  gcc show2.c -o show2 -Wall -lm 
 * run: show2 <filename> 0.708819
 *
 **********************************************************************/
...
int main(const int argc, const char **argv)
{
  ...
  unsigned long size, prevSize ;

  if (argc != 3)
    {
      printf ("%s <light curve file (2-column format)> <period>\n", argv[0]) ;
      return (1) ;
    }
  ...
  size = 0 ;
  rewind(finLC) ;
  while (2 == fscanf(finLC, "%lf %lf\n", &tmpTime, &tmpMag))
    if (size < prevSize)  // sign of invalid magnitude
      {
	time[size] = tmpTime ;
	mag[size] = tmpMag ;
	size++ ;
  ...
  if (size != prevSize)
    {
      printf ("ERROR: read number mismatch\n") ;
      free (mag) ;
      free (time) ;
      return (4) ;
    }
