 ...
 *
 * compile:  gcc showDebilNoFold.c -o showDebilNoFold -Wall -lm -O4
 *
 * run:      showDebilNoFold ag.dbl ag.sm
 *           
 *           sm
 *           : inp ag.sm
 *           : quit
...
#define NUM_PAGES_PER_LC 2

#define WRITE_CURVE_FIT_PER_LINE 2500   // The number of fitted points to write
...
int writeCurveFit (double *time, double *amp, double *errMag, int size, 
		   double period, double time0, double sin_i,
		   double r1, double r2, const double B1, const double B2,
		   double e, double omega, char *localFilename, double minTime, double maxTime)
{
  FILE *fout ;
  char foutName[256] ;
  int i ;
  ...
  for (i = 0 ; i < size ; i++)
    {
      F = flux (mod1(time[i] / period), time0, sin_i, r1, r2, B1, B2, e, omega, INTEGRATION_STEP) ;
  ...
  for (i = 0 ; i < (WRITE_CURVE_FIT_PER_LINE * NUM_BOXES_Y * NUM_PAGES_PER_LC) ; i++)
    {
      t = minTime + ((maxTime - minTime) * i / (double)((WRITE_CURVE_FIT_PER_LINE * NUM_BOXES_Y * NUM_PAGES_PER_LC) - 1)) ;

      F = flux (mod1(t / period), time0, sin_i, r1, r2, B1, B2, e, omega, INTEGRATION_STEP) ;
      fprintf (fout, "%f %f\n", t, -2.5 * log10(F)) ;
    }
...
void printSM (FILE *foutSM, char *filename, double period, double minTime, double maxTime)
{
  static int didRun = 0 ; 
  int i, j ;
  ...
  for (i = 0 ; i < NUM_PAGES_PER_LC ; i++)
      ...
      for (j = NUM_BOXES_Y ; j > 0 ; j--)
	{
	  ...
	  fprintf (foutSM, "limits %f %f $fy2 $fy1\n",      // invert axis direction
		   minTime + ((maxTime - minTime) * ((i * NUM_BOXES_Y) + NUM_BOXES_Y - j) / (double)(NUM_BOXES_Y * NUM_PAGES_PER_LC)),
		   minTime + ((maxTime - minTime) * (1 + (i * NUM_BOXES_Y) + NUM_BOXES_Y - j) / (double)(NUM_BOXES_Y * NUM_PAGES_PER_LC))) ;
	  // add margin:	   limits 0 1 $($fy2+0.2) $($fy1-0.1)
	  
	  fprintf (foutSM, "ctype blue\n") ;
	  fprintf (foutSM, "connect xf yf\n") ;
	  fprintf (foutSM, "ctype black\n") ;
	  fprintf (foutSM, "points x y\n") ;
	  
	  fprintf (foutSM, "ticksize 0 0 0 0\n") ;
	  fprintf (foutSM, "xlabel %s   (part %d of %d)\n", 
		   filename, 1 + (i * NUM_BOXES_Y) + NUM_BOXES_Y - j, NUM_BOXES_Y * NUM_PAGES_PER_LC) ;
...
int main (int argc, char **argv)
{
  ...
  double tmpTime, tmpMag, tmpErr, minTime = 0, maxTime = 0 ;
  double *time, *amp, *errMag ;
  ...
  while (10 == (res = fscanf (fin, "%s %lf %lf %*f %lf %*f %lf %*f %lf %*f %lf %*f %lf %*f %lf %*f %lf %*f %*d %*d %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f\n", filename, &period, &e, &r1, &r2, &B1, &B2, &sin_i, &time0, &omega)))
	...
	if (tmpErr > 0.0)  // Sign of an invalid magnitude
      ...
      amp = (double*)malloc (size * sizeof(double)) ;
      errMag = (double*)malloc (size * sizeof(double)) ;
	...
	if (tmpErr > 0.0)  // Sign of an invalid magnitude
	  {
		...
		if (tmpTime > minTime)  maxTime = tmpTime ;
	      }

	    //	    time[i] = (double)mod1(tmpTime / period) ;
	    time[i] = tmpTime ;
	    amp[i] = pow(10.0, -0.4 * tmpMag) ;  // Convert magnitude (logarithmic) to amplitude (linear)
	    errMag[i] = tmpErr ;
	    i++ ;
	  }
      ...
      if (i != size)
	{
      ...
      i = writeCurveFit (time, amp, errMag, size, period, time0, sin_i, r1, r2, B1, B2, e, omega, localFilename, minTime, maxTime) ;
       
      if (i)
	printf ("Warning: Couldn't write a *.data or *.fit file for '%s' (P=%.f3).", localFilename, period) ;
      else
	printSM (foutSM, localFilename, period, minTime, maxTime) ;
